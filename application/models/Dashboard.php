...
class Dashboard extends CI_Model {
	...
	{
		return $this->db->query("SELECT posts.id as post_id, content, posts.created_at as created_at, first_name, last_name  FROM posts JOIN users ON posts.user_id = users.id WHERE user_id = ? ORDER BY posts.created_at DESC", $userid)->result_array();
	}
	...
	public function insert_post($data)
	{
		...
		$values = array($this->session->userdata['wall_user'], $data['user_post'], date("Y-m-d H:i:s"), date("Y-m-d H:i:s"));
		$this->db->query($query, $values);
	}

	public function insert_comment($data)
	{
		$query = "INSERT INTO comments (message_id, content, created_at, updated_at) VALUES (?,?,?,?)";
		$values = array($data['post_id'], $data['user_comment'], date("Y-m-d H:i:s"), date("Y-m-d H:i:s"));
		$this->db->query($query, $values);
	}
